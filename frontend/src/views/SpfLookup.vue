...
import { spfLookup } from '../api/client';
...
const cliCommand = computed(() => buildCli(['nortools', 'spf', '--json', domain.value]));
...
async function lookup() {
  ...
  result.value = null;
  try {
    result.value = await spfLookup(domain.value);
  } catch (e: unknown) {
...
<template>
  <div class="tool-view">
    ...
    <p class="desc">Check SPF (Sender Policy Framework) records for a domain.</p>
