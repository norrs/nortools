...
import { httpCheck } from '../api/client';
...
const cliCommand = computed(() => buildCli(['nortools', 'http', '--json', url.value]));
const cliDisabled = computed(() => !url.value);
...
async function check() {
  ...
  result.value = null;
  try {
    result.value = await httpCheck(url.value);
  } catch (e: unknown) {
...
<template>
  <div class="tool-view">
    <h2>HTTP Check</h2>
    <p class="desc">Check HTTP response from a URL.</p>
    <form @submit.prevent="check" class="lookup-form">
      <input v-model="url" placeholder="URL or domain (e.g. example.com)" class="input" />
